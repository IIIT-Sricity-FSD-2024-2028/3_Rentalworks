...
async function sendNotification() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      notifHistory.unshift(savedNotif);
      
      // Fallback for mock simulation system compatibility
      let crossNotifs = JSON.parse(localStorage.getItem('cross_notifications') || '[]');
      crossNotifs.push(savedNotif);
    ...
    showToast('error', 'Error', 'Failed to connect to backend server');
  }
}
